...
<body style="text-align: center;background-color: #F5F5F5">
    ...
    <div style="background-color: white;width: 1000px;height:auto!important;_height:500px;min-height:500px;margin-left: 30px">
        <div class="div1" style="border-bottom:2px solid #E0E0E0" >
            <input id="title" name="title" style="border:0px;width: 950px;height: 49px;outline:none;font-size: 35px" type="text" placeholder="请输入章节号与章节名称。实例：“ 第十章  天外陨石”"><br/>
            <span id="stitle"></span>
        </div>
        <div>
            <input id="article_id" type="hidden" name="{{$id}}" value="{{$id}}">
        </div>
        <div>
            <br/>
            <textarea id="content" name="content" style="border:0px;width: 950px;height: 500px;outline:none;font-size: 25px" type="text" placeholder="章节内容"></textarea>
...
<script type="text/javascript">
    $('#button').click(function () {
        ...
        layer.confirm('请仔细检查您所要提交的文章后提交',function (index) {
            $.post('/author/savecontent',
                {'article_title':article_title,'article_content':article_content,'article_id':article_id, 'article_money':article_money},
                function (data) {
                    console.log(data);
                    if(data.status == 200){
